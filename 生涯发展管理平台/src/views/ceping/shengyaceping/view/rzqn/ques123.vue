<template>
      ...
      <div class="ques-wrapper test_detail test_detail_ing test_2">
        ...
        <div class="ques-container box d-relative">
          <div v-if="subject==3" class="ques-answer ques-biao-answer">
            <div :class="index==curIndexC ? 'ques-item ques-item-hide active': 'ques-item ques-item-hide'"
                 v-for="(question,index) in questionsC" :cid="question.id">
              <div class="q-title q-art-title">{{question.detail}}</div>
              <div class="q-title q-art-title">
                <img :src="question.img" alt="">
                <!--<p>（样图）</p>-->
              </div>
              <div class="q-answer q-biao-answer q-diy-answer q-art3-answer">
                <ul>
                  <li class="answer-item yin-answer-item" v-for="(opt,i) in question.question_options"
                      v-if="opt.opt_img && i==0 && index==2">
                  ...
                  <li class="answer-item yin-answer-item" v-for="(opt,i) in question.question_options"
                      v-if="opt.opt_img && i>2 && index==2" :answer="opt.opt_order" @click="answerSelect($event)">
                    <img :src="opt.opt_img" alt="">
                    <div class="group">
                      <div class="layui-unselect layui-form-radio" v-if="">
                        <i class="layui-anim layui-icon"></i>
                      </div>
                      <label class="" v-if="i==0">A</label>
                      <label class="" v-if="i==1">B</label>
                      <label class="" v-if="i==2">C</label>
                      <label class="" v-if="i==3">D</label>
                      <label class="" v-if="i==4">E</label>
                      <label class="" v-if="i==5">F</label>
                      <label class="" v-if="i==6">G</label>
                      <label class="" v-if="i==7">H</label>
                    </div>
                  </li>

                  <li class="answer-item yin-answer-item" v-for="(opt,i) in question.question_options"
                      v-if="opt.opt_img && index!=2" :answer="opt.opt_order" @click="answerSelect($event)">
                    <img :src="opt.opt_img" alt="">
                    <div class="group">
                      <div class="layui-unselect layui-form-radio" v-if="">
                        <i class="layui-anim layui-icon"></i>
          ...
          <div class="bottom next test-confirm clearfix" style="bottom:20px;">
            <a href="javascript:;" class="btn btn_start" v-if="tip==0" @click="answerClickC($event)">确定</a>
          </div>
        ...
        <div class="page">
          <em class="col_2">{{curIndexC+1}} </em>/{{totalC}}
        </div>
  ...
  <!--<img :src="opt.opt_img" alt="">-->
  <!--<label class="block-label" v-if="index==0">A</label>-->
  <!--<label class="block-label" v-if="index==1">B</label>-->
  <!--<label class="block-label" v-if="index==2">C</label>-->
  <!--<label class="block-label" v-if="index==3">D</label>-->
  <!--<label class="block-label" v-if="index==4">E</label>-->
  <!--<label class="block-label" v-if="index==5">F</label>-->
  <!--<label class="block-label" v-if="index==6">G</label>-->
  <!--<label class="block-label" v-if="index==7">H</label>-->
  <!--</li>-->
  <!--</ul>-->
  <!--</div>-->
  <!--</div>-->
  ...
  <!--&lt;!&ndash;<div class="seq-wrapper pull-right">题号:{{curIndex+1}}/{{total}}</div>&ndash;&gt;-->
  <!--<div class="seq-wrapper pull-right">题号:{{curIndexC+1}}/{{totalC}}</div>-->
    ...
    methods: {
      ...
      answerClickC: function (e) {
        ...
        if (this.curIndexC + 1 < this.totalC) {
          ...
          this.curIndexC++;
          $('.answer-item').removeClass('active');
          this.bar = Number(this.curIndexC) / Number(this.totalC) * 100;
        } else {
          console.log('结束测试')
          this.submitC();
        }
...
//              this.layerMsg('测试成功!');
                ...
                clearInterval(this.t);
                this.clearB=false;
              this.$router.push('/shengyaceping/rzqn/success/123')
            } else {
              this.layerMsg(data.msg);
            }
          }.bind(this),
