  ...
  <div class="pc-page">
    <div class="test_main test_main_2">
      ...
      <div class="d-relative test_detail test_2">
        <div class="radio_box ques-item" style="margin-bottom: 60px" v-for="(question,index) in questions" v-if="index==curIndex" :paper="question.paper"
             :cid="question.id">
          ...
          <div class="formbox clearfix">
            <div class="q-answer-ani2">
              <div class="answer-item" v-for="opt in question.question_options"
                   :answer="opt.id" @click="answerSelect($event)">
...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      quesList: function () {
        this.$ajax.post("/api/test/cognition/organize/list", {emulateJSON: true}).then(
          function (res) {
            // 请求成功的结果
      ...
      submit: function () {
        ...
        this.$ajax.post("/api/result/cognition/organize/compute", {
            time: this.time,
...
//              this.layerMsg('测试成功!')
              this.saveNode(2, 11);
              this.$router.push('/shengyaceping/rzqn/success/10')
            } else {
              this.layerMsg(data.msg);
            }
          }.bind(this),
