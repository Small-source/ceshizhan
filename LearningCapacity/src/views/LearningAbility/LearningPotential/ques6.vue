...
<script>
  ...
  export default {
    ...
    methods: {
       ...
       */
      quesList: function () {
        this.$ajax.post(this.G_uri+"/test/cognition/logic/list",{
            timeout:3000,
            headers: {
       ...
       */
      submit: function () {
        this.$ajax.post(this.G_uri+"/result/cognition/logic/compute", {
            time: this.time,
...
//              this.layerMsg('测试成功!')
              this.saveNode(1, 7);
              this.$router.push('/learningAbility/learningPotential/success/6')
            } else {
              this.layerMsg(data.msg);
            }
          }.bind(this),
